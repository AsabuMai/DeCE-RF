...
def make_specs(src: Path) -> tuple[EnvSpec, ...]:
    ...
    EnvSpec(
        ...
        "/usr/bin/python3.10",
        (("-e", str(src / "RF-Solver-Edit/FLUX_Image_Edit")),),
        "FLUX image-edit subproject install; video subproject is out of E2 image scope.",
    ),
    EnvSpec(
        "ReFlex",
        "reflex-py310",
    ...
    EnvSpec(
        ...
        "masactrl-py310",
        "/usr/bin/python3.10",
        (("-r", str(src / "MasaCtrl/requirements.txt")), ("torch", "torchvision", "pillow", "numpy", "huggingface-hub==0.13.4")),
    ...
    EnvSpec(
        ...
        "No requirements file; dependency set inferred from text-guided imports.",
    ),
    EnvSpec(
        "ledits_pp",
        "ledits-pp-py310",
