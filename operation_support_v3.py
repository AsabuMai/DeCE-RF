...
def build_container_interior_region(
    host_mask: torch.Tensor,
    center_width: float = 0.58,
    center_height: float = 0.58,
) -> torch.Tensor:
    host = normalize_spatial_map(host_mask)
    bbox = _binary_bbox(host)
    if bbox is None:
    ...
    sy1 = int(round(cy + 0.5 * center_height * bh))
    center = _box_mask_like(host, (sx0, sy0, sx1, sy1))
    interior = torch.minimum(host, center)
    if float(interior.detach().float().max().item()) <= 1e-6:
        interior = center
    return normalize_spatial_map(interior).to(device=host.device, dtype=host.dtype)
