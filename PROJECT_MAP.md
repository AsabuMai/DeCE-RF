...
```text
decoupled clean-estimate-space control for Rectified Flow local editing
```

Support-v3 is not the full method. In paper-facing language, it should be
treated as an operation-conditioned support interface that supplies the control
geometry:

```text
M_edit, M_core, M_ring, M_preserve
```

to the rmsgap clean-estimate-space controller. `support_v3_fixed` is the
baseline, and M22/core-target-transport is currently a side branch, not the
mainline.
