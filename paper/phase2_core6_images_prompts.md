...
| T5 | Localized same-color material replacement | `pillow_same_color_cable_knit` | `data/phase2_candidates/pexels_white_pillow_brown_sofa_6312089.jpg` | `add_decal` | `on_surface` | canonical T5 (2026-06-10); entire pillow surface to same-color white cable-knit, model-driven, not symbol/decal/recolor; corduroy panel is diagnostic |
...
### T5-1 `pillow_same_color_cable_knit` (canonical, 2026-06-10)
...
```text
attention: knit,cable,braided,pillow
changed: knit,cable,braided
SUPPORT_EDIT_OPERATION=add_decal
...
DECAL_SHAPE=waffle_panel  # weak structure hint only; ref guidance 0.08
DECAL_BOX=0.195,0.300,0.558,0.830  # full pillow; bright-host clip trims to surface
DECAL_COLOR=128,128,128
# Recipe: model-driven (hedit 0.68, text 0.14, rec 0.40, traj 0.25),
# no final reference composite, final source-chroma projection scale 1.0.
```

### T5-1-diagnostic `pillow_same_color_corduroy_panel` (superseded center-panel probe)

Previous canonical T5; kept for diagnostics. Center-panel corduroy edit was not
visible enough after VAE round-trip (see worklog 2026-06-10).
