...
Date: 2026-06-01 server evidence, reconciled locally on 2026-06-02

Scope: completed server evidence package under the previous Core-6 naming. This
package supersedes the earlier Core-5 quantitative summary by adding one
localized recolor/attribute-editing task, `red_chair_blue`, after seed-10 visual
gating and mask correction.

Important reconciliation: the current WACV experiment design now defines Core-6
as six operation categories:

```text
T1 attached accessory
T2 container-constrained spatial insertion
T3 small surface decal
T4 object-level recoloring
T5 surface pattern editing
T6 simple exposed-object removal
```

The server evidence package does not yet cover the updated strict T2/T5 rows
(`bowl_apple_inside`, `pillow_blue_stripes`). It should be reported as completed
evidence for T1/T3/T4/T6 plus auxiliary T1/T3 examples:

| Server task | Updated design role |
| --- | --- |
| `dog_sunglasses` | T1 canonical attached accessory |
| `cat_crown` | T1 secondary / teaser attached accessory |
| `mug_heart` | T3 canonical small surface decal |
| `tshirt_star` | T3/T5-adjacent clothing decal expansion; not final T5 surface-pattern row |
| `red_chair_blue` | T4 canonical recolor |
| `backpack_remove_toy_charm` | T6 canonical exposed-object removal |
...
Executed server matrix:

```text
6 server task instances x 4 paper-facing methods x 3 seeds = 72/72 complete
```

Fixed-control ablation table rows:

```text
6 server task instances x 2 compared methods x 3 seeds = 36 table rows
18 new fixed-control outputs plus reused DeCE-RF outputs
```
...
The server evidence package keeps the fixed rerun protocol and treats
`red_chair_blue` as the promoted recolor/attribute-edit case. DeCE-RF remains
the highest-overall method mean, driven by localized add/decal success and the
chair recolor case. Generic support remains a strong preservation baseline, but
it over-preserves on several add/decal tasks. Direct target guidance remains
aggressive and often changes source identity or composition.
...
ablation now covers the promoted server evidence task set. The fixed-vs-feedback
gap is modest, so this supports component evidence rather than a headline claim.
...
After the 2026-06-01 server evidence rerun:

1. Treat the 72 paper-facing server runs plus 18 fixed-control outputs as the
   current completed evidence package, not as the final updated strict Core-6.
2. Map the completed evidence into the updated design as T1/T3/T4/T6 evidence
   plus supplementary T1/T3 rows.
...
5. Do not promote `web_plate_apple` / bowl+apple insertion yet: the server
   add-editor probes show apple placement is pulled toward bowl rim or external
   shadows, so updated T2 still needs a geometry-aware interior placement prior.
6. The updated T5 row remains unimplemented as `pillow_blue_stripes`; the
   completed `tshirt_star` evidence should be reported as a clothing-decal
   expansion unless the paper explicitly narrows T5 to clothing/surface decal.
7. The next experimental priority is baseline coverage under the evidence
   ...
   resampling.
8. Existing controller stress results are enough for a modest feedback-control
