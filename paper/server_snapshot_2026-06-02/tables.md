...
```text
current: 6 core tasks x 4 paper-facing methods x seeds 10,11,12 = 72 complete runs
```
...
- is now complete for the promoted Core-6 task set.

## Expansion Tables

The Core-6 main table includes one recolor task gated by seed-10 visual inspection and a corrected fixed evaluation mask:

```text
selected: red_chair_blue
```

Weak replacement/removal candidates should move to a limitation/stress table rather than being tuned into the main table. `backpack_remove_toy_charm` remains in Core-6 as a target-removal success with a local zipper/fabric preservation caveat.
...
Current server check: external baseline source repositories have been downloaded under `/workspace/baselines/src`, with pinned clone status in `/workspace/baselines/download_status.tsv`. LEDITS++ has completed Core-6 seeds 10/11/12 as an external diffusion-editing baseline; other external baseline environments, model weights, and Core-6 output images are not validated yet. The old FlowEdit root and Python environment from the archived 2026-05-10 baseline scripts are missing, and no current `outputs/baselines` result images were found for the Core-6 task set.
...
Existing baseline artifacts are older core-4 evidence and should be reported as availability/qualitative context unless rerun under the Core-6 protocol.
