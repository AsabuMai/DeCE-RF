...
| T5 | Surface pattern editing | `pillow_blue_stripes` | `data/pretty_free_candidates/pexels_plain_pillow_sofa_phase1.jpg` | `add_decal` | `on_surface` | Phase 1 canonical; local pillow pattern |
...
```text
A photo of the same plain grey pillow with subtle blue horizontal stripes printed on its surface, with the pillow shape, sofa, table, wall, and background unchanged.
```
