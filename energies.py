...
def reconstruction_energy_latent(
    x0_pred: torch.Tensor,
    x_src: torch.Tensor,
    reduction: str = "mean",
) -> torch.Tensor:
    """
    Minimal latent-space reconstruction energy:

        E_rec = 1/2 || x_hat_0 - x_src ||^2

    This is the simplest stable starting point before introducing masks,
    feature-space losses, or region-aware weighting.
    """
    x0_pred, x_src, _ = align_rec_space(x0_pred, x_src, None)
    diff = x0_pred - x_src
    sq = 0.5 * diff.pow(2)
