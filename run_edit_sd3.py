...
def main() -> None:
    ...
    x_tar = HRecSD3Edit(
        ...
        mask_trimap_inner_erode_kernel=args.mask_trimap_inner_erode_kernel,
        mask_trimap_outer_dilate_kernel=args.mask_trimap_outer_dilate_kernel,
        mask_trimap_boundary_edit_scale=args.mask_trimap_boundary_edit_scale,
        mask_trimap_boundary_preserve_scale=args.mask_trimap_boundary_preserve_scale,
    ...
    final_ref_composite_applied = False
    if (
        args.final_ref_composite_scale > 0.0
        and args.final_ref_composite_image
        and args.final_ref_composite_mask
    ):
        result_image = image_tar[0].convert("RGB")
        ref_image = PIL.Image.open(args.final_ref_composite_image).convert("RGB").resize(
            result_image.size,
            PIL.Image.Resampling.LANCZOS,
        )
        ref_mask = PIL.Image.open(args.final_ref_composite_mask).convert("L").resize(
            result_image.size,
            PIL.Image.Resampling.LANCZOS,
        )
        if args.final_ref_composite_mask_blur > 0.0:
            ref_mask = ref_mask.filter(PIL.ImageFilter.GaussianBlur(args.final_ref_composite_mask_blur))
        alpha = np.asarray(ref_mask, dtype=np.float32)[..., None] / 255.0
        alpha *= max(0.0, min(1.0, args.final_ref_composite_scale))
        result_arr = np.asarray(result_image, dtype=np.float32)
        ref_arr = np.asarray(ref_image, dtype=np.float32)
        blended = result_arr * (1.0 - alpha) + ref_arr * alpha
        image_tar[0] = PIL.Image.fromarray(blended.clip(0, 255).round().astype(np.uint8), mode="RGB")
        final_ref_composite_applied = True
    final_chroma_source_applied = False
    if args.final_chroma_source_scale > 0.0:
        chroma_mask_path = args.final_chroma_source_mask or args.final_ref_composite_mask
        if chroma_mask_path:
            result_image = image_tar[0].convert("RGB")
            chroma_src_image = PIL.Image.open(args.image).convert("RGB").resize(
                result_image.size,
                PIL.Image.Resampling.LANCZOS,
            )
            chroma_mask = PIL.Image.open(chroma_mask_path).convert("L").resize(
                result_image.size,
                PIL.Image.Resampling.LANCZOS,
            )
            if args.final_chroma_source_mask_blur > 0.0:
                chroma_mask = chroma_mask.filter(PIL.ImageFilter.GaussianBlur(args.final_chroma_source_mask_blur))
            alpha = np.asarray(chroma_mask, dtype=np.float32)[..., None] / 255.0
            alpha *= max(0.0, min(1.0, args.final_chroma_source_scale))
            res_arr = np.asarray(result_image, dtype=np.float32)
            src_arr = np.asarray(chroma_src_image, dtype=np.float32)
            res_luma = 0.299 * res_arr[..., 0] + 0.587 * res_arr[..., 1] + 0.114 * res_arr[..., 2]
            src_luma = 0.299 * src_arr[..., 0] + 0.587 * src_arr[..., 1] + 0.114 * src_arr[..., 2]
            res_chroma = res_arr - res_luma[..., None]
            src_chroma = src_arr - src_luma[..., None]
            blended_chroma = res_chroma * (1.0 - alpha) + src_chroma * alpha
            out_arr = blended_chroma + res_luma[..., None]
            image_tar[0] = PIL.Image.fromarray(out_arr.clip(0, 255).round().astype(np.uint8), mode="RGB")
            final_chroma_source_applied = True
    final_outside_restore_applied = False
    if args.final_outside_restore_scale > 0.0:
        restore_mask_path = args.final_outside_restore_mask or args.final_chroma_source_mask or args.final_ref_composite_mask
        if restore_mask_path:
            result_image = image_tar[0].convert("RGB")
            restore_src_image = PIL.Image.open(args.image).convert("RGB").resize(
                result_image.size,
                PIL.Image.Resampling.LANCZOS,
            )
            restore_mask = PIL.Image.open(restore_mask_path).convert("L").resize(
                result_image.size,
                PIL.Image.Resampling.LANCZOS,
            )
            if args.final_outside_restore_mask_blur > 0.0:
                restore_mask = restore_mask.filter(PIL.ImageFilter.GaussianBlur(args.final_outside_restore_mask_blur))
            inside = np.asarray(restore_mask, dtype=np.float32)[..., None] / 255.0
            outside_alpha = (1.0 - inside) * max(0.0, min(1.0, args.final_outside_restore_scale))
            res_arr = np.asarray(result_image, dtype=np.float32)
            src_arr = np.asarray(restore_src_image, dtype=np.float32)
            out_arr = res_arr * (1.0 - outside_alpha) + src_arr * outside_alpha
            image_tar[0] = PIL.Image.fromarray(out_arr.clip(0, 255).round().astype(np.uint8), mode="RGB")
            final_outside_restore_applied = True
    ...
    metadata = {
        ...
        **proposal_mask_metadata,
        ...
        "mask_blend_mode": args.mask_blend_mode,
        "final_preserve_box": final_preserve_box,
        "final_ref_composite_image": args.final_ref_composite_image,
        "final_ref_composite_mask": args.final_ref_composite_mask,
        "final_ref_composite_scale": args.final_ref_composite_scale,
        "final_ref_composite_mask_blur": args.final_ref_composite_mask_blur,
        "final_ref_composite_applied": final_ref_composite_applied,
        "final_chroma_source_scale": args.final_chroma_source_scale,
        "final_chroma_source_mask": args.final_chroma_source_mask,
        "final_chroma_source_mask_blur": args.final_chroma_source_mask_blur,
        "final_chroma_source_applied": final_chroma_source_applied,
        "final_outside_restore_scale": args.final_outside_restore_scale,
        "final_outside_restore_mask": args.final_outside_restore_mask,
        "final_outside_restore_applied": final_outside_restore_applied,
