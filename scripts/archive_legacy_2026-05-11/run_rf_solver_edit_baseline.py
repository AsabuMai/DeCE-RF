...
def find_result(output_dir: Path, prefix: str) -> Path:
    candidates = sorted(output_dir.glob("img_*.jpg"))
    return candidates[-1] if candidates else output_dir / "img_0.jpg"
...
def complete_row(
    ...
    *,
    ...
    rf_solver_root: Path,
    rf_solver_result: Path,
    command_text: str,
) -> dict[str, str]:
    Image.open(rf_solver_result).convert("RGB").save(run_dir / "result.png")
    matched_conditions = (
        "official RF-Solver-Edit FLUX-dev runner; source image resized with max_image_size=512 "
        "and cropped to multiples of 16; source prompt and target prompt match; "
        "official RF-Solver image-editing parameters num_steps=15, inject=3, guidance=2, "
        "offload=True; seed requested by manifest but the public RF-Solver image script "
        "does not expose seed control"
    )
    metadata = {
        "baseline": "rf_solver_edit",
        "task": task,
        "seed": int(seed),
    ...
    row.update(
        {
            ...
            "matched_conditions": matched_conditions,
            "failure_reason": "",
            "notes": "official RF-Solver-Edit FLUX-dev runner",
        }
    )
    return row
...
def run_row(
    ...
    *,
    ...
    edit_py = rf_solver_root / "src" / "edit.py"
    cmd = [
        python_bin,
        str(edit_py),
        "--source_prompt",
    ...
    env = {"CUDA_VISIBLE_DEVICES": device, "PYTORCH_CUDA_ALLOC_CONF": "expandable_segments:True"}
    command_text = (
    ...
    existing_result = find_result(output_dir, "")
    if existing_result.exists() and not dry_run:
        return complete_row(
            ...
            rf_solver_root=rf_solver_root,
            rf_solver_result=existing_result,
    ...
    try:
        ...
        if completed.returncode != 0:
            raise RuntimeError(f"RF-Solver-Edit exited with code {completed.returncode}")
        result = find_result(output_dir, "")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run matched RF-Solver-Edit FLUX-dev baselines.")
    parser.add_argument("--manifest", default="experiments/baseline_parity_manifest.csv", type=Path)
    parser.add_argument("--rf-solver-root", default="/home/Wu_25R8111/RF-Solver-Edit/FLUX_Image_Edit", type=Path)
    ...
    for index, row in enumerate(rows):
        if row.get("baseline") != "rf_solver_edit":
            continue
        ...
        rows[index] = run_row(
            row,
            repo_root=repo_root,
            rf_solver_root=args.rf_solver_root,
    ...
    print(f"processed {selected} RF-Solver-Edit row(s)")
    return 0
