...
def source_command(seed: int) -> tuple[dict[str, str], list[str]]:
    command_path = (
        ROOT
        / "outputs"
        / "pretty_matrix"
...
def build_command(seed: int, variant: Variant, method: str) -> tuple[Path, dict[str, str], list[str]]:
    src_env, cmd = source_command(seed)
    cmd = list(cmd)
    ...
    for seed in seeds:
        for variant in VARIANTS:
            for method in METHODS:
                out_dir, env, cmd = build_command(seed, variant, method)
