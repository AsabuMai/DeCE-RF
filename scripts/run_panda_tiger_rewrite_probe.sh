...
run_case() {
  ...
  local out_dir="${ROOT}/outputs/panda_tiger_rewrite_${name}"
  mkdir -p "${out_dir}/masks"
  if [[ "${FORCE}" != "1" && -f "${out_dir}/result.png" && -f "${out_dir}/stats.json" ]]; then
    echo "[skip] panda_tiger_rewrite_${name} already exists"
    return
  fi
...
run_case subject_noise025 \
  --edit-initial-noise-scale 0.25 \
  --edit-initial-noise-region subject

run_case subject_noise050 \
  --edit-initial-noise-scale 0.50 \
  --edit-initial-noise-region subject

run_case core_noise050 \
  --edit-initial-noise-scale 0.50 \
  --edit-initial-noise-region core

run_case subject_noise035_anchor \
  --edit-initial-noise-scale 0.35 \
  --edit-initial-noise-region subject \
  --edit-guidance-scale 0.5
